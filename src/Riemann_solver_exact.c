/*
 * This is an implementation of the exact Riemann solver for two dimensional
 * Euler equtions
 *                    w_t + f(w)_x + g(w)_y = 0
 * where
 *       / rho   \      / rho*u     \      / rho*v     \
 ...
 *       \ E     / ,    \ u*(E+p)   / ,    \ v*(E+p)   / ,
 * where E=0.5*rho*(u^2+v^2) + p/(\gamma-1) and \gamma is the heat ratio. The
 * left and right states are allowed to commit to different \gamma.
 * 
 * The present program is implemented by Dr. Zhifang Du based on the algorithm
 * stated in [1].
 *
 * [1] M. Ben-Artzi and J. Falcovitz, Generalized Riemann Problems in Computa-
 *     tional Fluid Dynamics, Cambridge University Press, Cambridge, 2003.
 */
...
double Riemann_solver_exact(double * U_star, double * P_star, double gammaL, double gammaR, double u_L, double u_R, double p_L, double p_R, double c_L, double c_R, double dist, int * CRW, double eps, double tol, int N)
{
  ...
  double muL, nuL, sigmaL, gammaL_1, gamma_p_L, _over_gamma_p_L, _over_gammaL_1;
  double muR, nuR, sigmaR, gammaR_1, gamma_p_R, _over_gamma_p_R, _over_gammaR_1;
  //double c_L, c_R;
  ...
  gammaL_1 = gammaL - 1.0;
  _over_gammaL_1 = 1.0 / gammaL_1;
  gamma_p_L = gammaL*p_L;
  _over_gamma_p_L = 1.0 / gamma_p_L;
  muR = (gammaR-1.0) / (2.0*gammaR);
  nuR = (gammaR+1.0) / (2.0*gammaR);
  sigmaR = (gammaR - 1.0) / (gammaR + 1.0);
  gammaR_1 = gammaR - 1.0;
  _over_gammaR_1 = 1.0 / gammaR_1;
  gamma_p_R = gammaR*p_R;
  _over_gamma_p_R = 1.0 / gamma_p_R;
  ...
  if(dist < eps)
  {
    ...
    *U_star = 0.5*(u_L + u_R);
    CRW[0] = 1;
    CRW[1] = 1;
    return ((u_L > u_R) ? (u_L-u_R) : (u_R-u_L));
  }
  ...
  if(p_R > p_L) // (u_LR,p_R) lies on the shock branch of I1
  {
    ...
    u_LR = delta_p * _over_gamma_p_L * (c_L / u_LR);
    u_LR = u_L - u_LR;
  }
  else // (u_LR,p_R) lies on the rarefaction branch of I1
  {
    u_LR = pow(p_R/p_L, muL) - 1.0;
    u_LR = 2.0 * c_L * u_LR * _over_gammaL_1;// (gammaL-1.0);
    u_LR = u_L - u_LR;
  }
  ...
  if(p_L > p_R) // (u_RL, p_L) lies on the shock branch of I3
  {
    ...
    u_RL = delta_p * _over_gamma_p_R * (c_R / u_RL);
    u_RL = u_R + u_RL;
  }
  else // (u_RL, p_L) lies on the rarefaction branch of I3
  {
    u_RL = pow(p_L/p_R, muR) - 1.0;
    u_RL = 2.0 * c_R * u_RL * _over_gammaR_1;// (gammaR-1.0);
    u_RL = u_R + u_RL;
  }
  if(u_LR > u_R+eps)
    CRW[1] = 0;
  ...
  k1 = -c_L * _over_gamma_p_L;//the (p,u)-tangent slope on I1 at (u_L,p_L), i.e. [du/dp](p_L)
  k3 =  c_R * _over_gamma_p_R;//the (p,u)-tangent slope on I3 at (u_R,p_R), i.e. [du/dp](p_R)

  p_INT = (k1*p_L - k3*p_R - u_L + u_R) / (k1 - k3);
  /*
   * The above formula computes the intersect of (u-u_L)=k1*(p-p_L) and (u-u_R)=k3*(p-p_R).
   * However, if (u_L,p_L)==(u_R,p_R), we have k1=-k3=k. The result is
   * (k*p+k*p)/(k+k), which may introduce additional error.
   */

  if(p_INT < 0)
    p_INT = 0.5*((p_L<p_R)? p_L : p_R);
  ...
  if(p_INT > p_L)
  {
    ...
    v_L = delta_p * _over_gamma_p_L * (c_L / v_L);
    v_L = u_L - v_L;
  }
  else
  {
    v_L = pow(p_INT/p_L, muL) - 1.0;
    v_L = 2.0 * c_L * v_L * _over_gammaL_1;// (gammaL-1.0);
    v_L = u_L - v_L;
  }
  if(p_INT > p_R)
  {
  ...
  else
  {
    ...
    v_R = 2.0 * c_R * v_R * _over_gammaR_1;// (gammaR-1.0);
    v_R = u_R + v_R;
  }
  gap = fabs(v_L - v_R);
  ...
  while((gap > tol) && (n != N))
  {
    ...
    if(p_INT > p_L)
    {
      ...
      temp2 = c_L * _over_gamma_p_L;// gammaL / p_L;
    ...
    if(p_INT > p_R)
    {
      ...
      temp2 = c_R * _over_gamma_p_R;// gammaR / p_R;
    ...
    gap = (v_L > v_R) ? (v_L-v_R) : (v_R-v_L);//fabs(v_L - v_R);
  }
//printf("n=%d\tN=%d\n", n, N);
